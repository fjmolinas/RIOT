 ...
 *
 * @file
 * @brief       ztimer6464_msg test application
 *
...
void *timer_thread(void *arg)
{
    ...
    while (1) {
         ...
         */
        printf("now=%"PRIu64":%"PRIu64" -> every %"PRIu64".%"PRIu64"s: %s\n",
               (uint64_t)(now / TICKS_PER_SEC),
               (uint64_t)(now % TICKS_PER_SEC),
               (uint64_t)(tmsg->interval / TICKS_PER_SEC),
               (uint64_t)(tmsg->interval % TICKS_PER_SEC),
               tmsg->text);

        tmsg->msg.type = 12345;
        tmsg->msg.content.ptr = tmsg;
        ztimer64_set_msg(ZTIMER64, &tmsg->timer, tmsg->interval, &tmsg->msg, thread_getpid());
    }
}
...
void *timer_thread_local(void *arg)
{
    ...
    while (1) {
        ...
        uint64_t now = ztimer64_now(ZTIMER64);
        int64_t sec = now / TICKS_PER_SEC;
        int64_t min = sec / 60;
        int64_t hr  = sec / 3600;
        printf("sec=%lld min=%lld hour=%lld\n", sec, min, hr);
    }
}

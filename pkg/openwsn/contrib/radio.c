...
int openwsn_radio_init(netdev_t *netdev)
{
    ...
    DEBUG_PUTS("[OpenWSN radio]: initialize riot-adaptation");
    openwsn_radio.dev = netdev;

    if (netdev->driver->init(netdev)) {
        DEBUG_PUTS("[OpenWSN radio]: unable to initialize device");
        return -1;
    }
    netdev->event_callback = _event_cb;

    DEBUG_PUTS("[OpenWSN radio]: put radio in standby");
    netopt_state_t state = NETOPT_STATE_STANDBY;
    netdev->driver->set(netdev, NETOPT_STATE, &(state), sizeof(state));

    DEBUG_PUTS("[OpenWSN radio]: set needed netdev options");
    ...
    /* Sniffer mode only */
    enable = NETOPT_ENABLE;
    netdev->driver->set(netdev, NETOPT_PROMISCUOUSMODE, &(enable),
                        sizeof(enable));
...
void radio_loadPacket(uint8_t *packet, uint16_t len)
{
    ...
    if (openwsn_radio.dev->driver->send(openwsn_radio.dev, &pkt) < 0) {
        DEBUG_PUTS("[OpenWSN radio]: couldn't load pkt");
    }
    DEBUG_PUTS("[OpenWSN radio]: loaded radio packet");
}
...
void radio_txNow(void)
{
    ...
    if (openwsn_radio.startFrame_cb != NULL) {
        val = sctimer_readCounter();
        openwsn_radio.startFrame_cb(val);
    }
}
                            ...
                            bool *crc)
{
    ...
    /* FCS is skipped by netdev in the returned length, but OpenWSN includes
       IEEE802154_FCS_LEN n its length value */
    *lenRead = nread + IEEE802154_FCS_LEN;

    /* get rssi, lqi & crc */
    *rssi = rx_info.rssi;
    *lqi = rx_info.lqi;
    openwsn_radio.dev->driver->get(openwsn_radio.dev, NETOPT_INTEGRITY_CHECK,
                                   crc, sizeof(crc));
...
static void _event_cb(netdev_t *dev, netdev_event_t event)
{
    ...
    if (event == NETDEV_EVENT_ISR) {
        ...
        DEBUG("[OpenWSN radio]: event triggered -> %i\n", event);
        switch (event) {
            case NETDEV_EVENT_RX_STARTED:
                openwsn_radio.startFrame_cb(_txrx_event_capture_time);
                DEBUG_PUTS("[OpenWSN radio]: NETDEV_EVENT_RX_STARTED");
                break;
            case NETDEV_EVENT_RX_COMPLETE:
                openwsn_radio.endFrame_cb(_txrx_event_capture_time);
                DEBUG_PUTS("[OpenWSN radio]: NETDEV_EVENT_RX_COMPLETE");
                break;
            case NETDEV_EVENT_CRC_ERROR:
                openwsn_radio.endFrame_cb(_txrx_event_capture_time);
                DEBUG_PUTS("[OpenWSN radio]: NETDEV_EVENT_CRC_ERROR");
                break;
            case NETDEV_EVENT_TX_COMPLETE:
                openwsn_radio.endFrame_cb(_txrx_event_capture_time);
                DEBUG_PUTS("[OpenWSN radio]: NETDEV_EVENT_TX_COMPLETE");
                break;
            default:
                break;
        }

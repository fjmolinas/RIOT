...
static void _print_cell_usage(void)
{
    ...
    puts(
        "\tcell add <slot_offset> <channel_offset> <adv|tx|rx> [<address>]: add cell directly to schedule");
    puts(
        "\tcell rmv <slot_offset> <channel_offset> <adv|tx|rx> [<address>]: remove cell directly in schedule ");
}
...
static int _6top_manage_cmd(int argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "add")) {
        ...
        if (!schedule_getCellsToBeRelocated(&neigh, celllist_delete) || \
            !msf_candidateAddCellList(celllist_add, num)) {
...
static int _6top_cmd(int argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "clear")) {
        ...
        if (argc == 3) {
            neigh.type = ADDR_64B;
            size_t len = netif_addr_from_str(argv[2], neigh.addr_64b);
            if (len == 0) {
                puts("Error: invalid address");
                return -1;
            }

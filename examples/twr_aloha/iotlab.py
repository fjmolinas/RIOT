...
class IoTLABExperiment:
    ...
    def board_from_iotlab_node(iotlab_node):
        ...
        if match is None:
            raise ValueError(
                "Unable to parse {} as IoT-LAB node name of "
                "format <node-name>.<site-name>.iot-lab.info".format(iotlab_node)
            )
    ...
    def _archi_from_board(board):
        ...
        return "{}:{}".format(
            IoTLABExperiment.BOARD_ARCHI_MAP[board]["name"],
    ...
    def _check_site(site):
        if site not in IoTLABExperiment.SITES:
            raise ValueError(
                "iotlab site must be one of {}".format(IoTLABExperiment.SITES)
            )

    @staticmethod
    def _valid_addr(env, addr):
        """Check id addr matches a specific BOARD"""
        return addr.startswith(IoTLABExperiment.BOARD_ARCHI_MAP[env["BOARD"]]["name"])
        ...
        for env in envs:
            ...
            if "BOARD" in env:
                if not IoTLABExperiment.valid_board(env["BOARD"]):
                    raise ValueError("{} BOARD unsupported in iotlab".format(env))
                if "IOTLAB_NODE" in env:
                    IoTLABExperiment.valid_iotlab_node(
                        env["IOTLAB_NODE"], site, env["BOARD"]
                    )
            elif "IOTLAB_NODE" in env:
                IoTLABExperiment.valid_iotlab_node(env["IOTLAB_NODE"], site)
                board = IoTLABExperiment.board_from_iotlab_node(env["IOTLAB_NODE"])
                env["BOARD"] = board
            else:
    ...
    def start(self, duration=60):
        ...
        logging.debug(
            'waiting for experiment {} to go to state "Running"'.format(self.exp_id)
        )
        self._wait()
